...
$(() => import('./z80/parser.js').then(z80parse => { import('./z80/expr.js').then(z80expr => {
    ...
    for (let node of program) {
        if (node.block === true) {
            ...
            console.log('Adding symbol', node.label);
            if (table.hasOwnProperty(node.label)) {
                error(node, 'redefined');
            } else if (node.label in table) {
                error(node, 'reserved');
            } else {
                lastsym = table[node.label] = {
                    org: org,
                    node: node,
                    value: org,
                }
            }
        } else if (node.equ !== undefined && lastsym !== undefined) {
            lastsym.value = node.equ;
            if (node.equ.expression !== undefined) {
                console.log('Updating to unresolved expression', node.equ);
                unresolved.add(lastsym);
            }
            lastsym = undefined;
        } else {
            lastsym = undefined;
        ...
        for (let sym of Object.entries((node.env ?? {}))) {
            console.log('Existing symbol', sym, node.env);
    ...
    while (unresolved.size > 0) {
        ...
        for (let symbol of unresolved) {
            ...
            for (let l of symbol.value.vars) {
                ...
                console.log(l, v, unresolved.has(v));
                // this works for just a simple label, but not for stacked environments
                if (unresolved.has(v)) {
                    resolvable = false;
                }
            }
            console.log('Resolving symbol', symbol, symbol.value.vars, resolvable);
            if (resolvable) {
